  ...
  <div class="capacity-list-box">
    ...
    <div class="capacity-list-par">
      <div class="capacity-her-keyword-box">
        <span class="capacity-her-ti"> 分类：</span>
        <span class="capacity-her-keyword">{{categoriesVim}}</span>
      </div>
      <div class="article-list-capacity-box float-left clear-fix">
        <div class="article-list-capacity clear-fix" v-for="(item,index) in categoriesList":key="index">
  ...
  import * as dateUtils from '../../utils/date'
    export default {
      ...
      methods:{
        ...
        categoriesListCode(){
          this.categoriesVim = this.$route.query.portalVim;
            api.categoryList( this.categoriesVim,this.pageNavigations.currentPage,
              this.pageNavigations.pageSize).then(result=>{
                  if (result.data.code=api.success_code){
                    this.categoriesList=result.data.data.articleList[0].records;
